...
public class Car {
	 ...
	 */
	public Car(String model1, int year1, String make1, String color1, double price1, int mileage1, int mpg1,
			String id1) {
		model = model1;
		year = year1;
		make = make1;
	...
	/**
	 * This is a search method that retrieve all of the cars that have the given
	 * parameter
	 * 
	 * @param cars
		...
		select.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (sorted_cars.size() != 0) {
					...
					JOptionPane.showMessageDialog(null, "Sorry, we do not have any cars that match the given criteria.",
							"Alert", JOptionPane.INFORMATION_MESSAGE, img);
					search_frame.dispose();
				}
			}
		});
	...
	public static void searchByMake(ArrayList<Car> cars) {
		...
		// add to this list.
		// maybe make it loop through the list and only display vehicle makes
		// that we have.
	...
	public static void searchByPrice(ArrayList<Car> cars) {
		...
		// add to this list.
		// maybe make it loop through the list and only display vehicle prices
		// that we have.
		...
		JButton select = new JButton("Select");
		select.setFont(new Font("Harlow PLAIN Italic", Font.PLAIN, 18));
		select.setForeground(secondary);
		...
		select.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				while (iter.hasNext()) {
					...
					// will add cars that are as good or better than the
					// selected MPG.
					if (car_MPG <= MPG) {
						sorted_cars.add(car);
					}
	...
	/**
	 * This is a method that will create a menu so the user can select what
	 * criteria he would like to search by.
	 * 
	 * @param cars
	 *            is the array list of cars that contains every car.
	 */
